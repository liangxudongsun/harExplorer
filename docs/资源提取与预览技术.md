...
| 能力         | Cocos Creator | Pragmatic | Slotmill |
| ---------- | ------------- | --------- | -------- |
| 纹理浏览 / 分类  | ✓             | ✓         | ✓        |
| Spine 动画预览 | ✓             | —         | —        |
| Spine 转序列帧 | ✓             | —         | —        |
...
```js
{ engine: 'cocos' | 'pragmatic' | 'slotmill', score, confident }
```

主要信号：

- **URL 模式**：Cocos 的 `/import/xx/`、`/native/`；Pragmatic / Slotmill 域名与资源路径特征。
- **内容嗅探**：少量 JS/JSON 正文中出现 `cc._RF.push`、`PIXI`、`slotmill` 等关键字。
...
2. 恢复 `skeleton.json` + `.atlas` 文本，并按 atlas 页名匹配 `/native/` 贴图。
...
- Viewer 内：`spine-bake.js`（可选多种精度管线：标准、高分辨率、超采样、最近邻等）。
