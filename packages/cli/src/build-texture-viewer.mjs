#!/usr/bin/env node
/**
 * Build a static texture gallery from a HAR file.
 * Usage:
...
if (!harPath) {
  console.error('Usage: node build-texture-viewer.mjs <file.har> [--out dir]');
  process.exit(1);
}
...
function getDataUrl(entry) {
  const content = entry.response?.content ?? {};
  const mime = (content.mimeType ?? 'application/octet-stream').split(';')[0];
  if (!content.text) return null;
  if (content.encoding === 'base64') {
    return `data:${mime};base64,${content.text}`;
  }
  if (mime.startsWith('image/') || mime === 'text/html') return null;
  return null;
}
...
const assetsDir = join(OUT_DIR, 'embedded');
mkdirSync(assetsDir, { recursive: true });

// Atlas pages → dimensions map
const atlasPages = new Map(); // pageName -> { width, height, regions, atlasPath }
...
for (const entry of entries) {
  ...
  urlMap.set(url, {
    ...
    mime,
    size,
    sizeFmt: size >= 1048576 ? `${(size / 1048576).toFixed(2)} MB` : size >= 1024 ? `${(size / 1024).toFixed(1)} KB` : `${size} B`,
...
const catalog = {
  ...
  tabs: [{
    id: 'default',
    label: manifest.meta.pageTitle || basename(harPath, '.har'),
    meta: manifest.meta,
...
console.log(JSON.stringify({
  ok: true,
  out: OUT_DIR,
  index: join(OUT_DIR, 'index.html'),
  textures: textures.length,
  embedded: manifest.meta.embedded,
  remote: manifest.meta.remote,
}, null, 2));
