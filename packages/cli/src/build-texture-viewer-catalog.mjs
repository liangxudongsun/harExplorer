...
async function main() {
  ...
  for (const src of sources) {
    ...
    let tab;
    if (src.type === 'pragmatic') {
      tab = buildPragmaticTab(harPath, OUT_DIR, src);
    } else if (src.type === 'cocos') {
      tab = buildCocosTab(harPath, OUT_DIR, src);
    } else if (src.type === 'gameart') {
      tab = buildGameartTab(harPath, OUT_DIR, src);
    } else {
      // Slotmill builder may fetch remote AVIF pages omitted from HAR
      tab = await buildSlotmillTab(harPath, OUT_DIR, src);
    }
    tabs.push(tab);
  }
  ...
  for (const extra of ['spine37-player.html', 'spine4-player.html', 'spine-bake.js']) {
    ...
    JSON.stringify(
      {
        ...
        tabs: tabs.map((t) => ({
          id: t.id,
          label: t.label,
          textures: t.meta.total,
          spine: t.animationManifest?.length ?? 0,
          audio: t.audioManifest?.length ?? 0,
          fonts: t.fontManifest?.length ?? 0,
        })),
      },
      null,
      2
